  ...
  def prepare_model(self):
    with tf.variable_scope("LSTMTDNN"):
      ...
      with tf.variable_scope("CNN") as scope:
        ...
        char_indices = tf.split(axis=1, num_or_size_splits=self.seq_length, value=self.char_inputs)
        word_indices = tf.split(axis=1, num_or_size_splits=self.seq_length, value=tf.expand_dims(self.word_inputs, -1))
        ...
        for idx in xrange(self.seq_length):
          ...
          if self.use_char:
            ...
            if self.use_word:
              ...
              cnn_output = tf.concat(axis=1, values=[char_cnn.output, tf.squeeze(word_embed, [1])])
            else:
      ...
      with tf.variable_scope("LSTM") as scope:
        ...
        loss = 0
        true_outputs = tf.split(axis=1, num_or_size_splits=self.seq_length, value=self.true_outputs)
